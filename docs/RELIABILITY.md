...
- `/robots.txt` — repo root, served as-is by GitHub Pages. Allows all
  user agents and points at the sitemap. Not parsed by Jekyll past the
  empty front-matter fence; do not put Liquid logic here beyond the
  `absolute_url` filter for the sitemap line.
- `/llms.txt` — repo root. Hand-curated Markdown summary for LLM-based
  discovery (the [llmstxt.org](https://llmstxt.org) convention).
  Iterates over `site.posts` via Liquid so the post list stays in sync.
