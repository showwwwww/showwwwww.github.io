...
如果只是把 DevTools 接入一个传统 Web 项目，前面的 wall 替换通常已经够了。
但我的场景还需要再多做一步，因为模拟器并不是一个普通的浏览器页面。它模拟的
是小程序运行环境，内部拆成了服务线程和渲染线程。

默认情况下，React DevTools backend 会在自己所在的运行时里绘制 overlay。
但在这个项目里，backend 所在的位置并不是用户真正看到和操作的界面。被选中的
Fiber 需要高亮到模拟器的渲染线程里。

![React DevTools backend agent 转发 overlay 消息](/assets/img/posts/react-devtools-custom-embed/react-agent.png)

![React DevTools 通过 DOM id 查找 overlay 目标](/assets/img/posts/react-devtools-custom-embed/react-overlay-index.png)

上面两张图展示的是这中间额外的一跳。在 backend 模块里，已有的 agent 负责在
不同运行时之间转发 DevTools 消息。在我的项目里，渲染线程中的真实 DOM 节点都
有一个 id，并且这个 id 能和对应的 Fiber 匹配。所以我改的是
`emit('inspectElement', ...)` 传入的参数，把原本选中的 Fiber 目标换成选中的
DOM id。agent 再把这个 id 转发给 React DevTools 用户界面，让用户界面可以检查并展示从
模拟器里选中的元素。
